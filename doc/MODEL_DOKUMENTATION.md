...
# Power BI Performance Dashboard – Modell- und Dokumentationsverzeichnis

Dieses Dokument vereint die technische Dokumentation und die vollständige Modellbeschreibung für das Power BI Performance Dashboard-Projekt.
...
### Modelldatei
Informationen zur Modelldatei (`Model.bim`) und Hinweise zur Bearbeitung mit Power BI Desktop, Tabular Editor und DAX Studio.

### Zweck und Nutzen
Erläuterung, wie die Dokumentation Entwicklern, Analysten und Stakeholdern hilft.
...
2. Verwenden Sie Tabular Editor (Open-Source-Tool), um sich mit dem laufenden Power BI Desktop-Prozess zu verbinden.
...
Das Datenmodell ist um Geschäftsprozesse herum organisiert mit klar getrennten **Faktentabellen** (enthält messbare Transaktionen) und **Dimensionstabellen** (enthält beschreibende Attribute). Das Modell folgt einem **Sternschema**-Designmuster, optimiert für analytische Abfragen und Business Intelligence Reporting.
...
1. **Mit Power BI Desktop**:
   - Öffnen Sie die Projektdatei `Performance Dashboard.pbip`.
...
2. **Mit Tabular Editor** (empfohlen für erweiterte Modellanalyse):
   - Laden Sie Tabular Editor herunter
   - Öffnen Sie das Power BI-Projekt (`.pbip`).
   - Starten Sie Tabular Editor über den Tab "Externe Tools" in Power BI Desktop.
   - oder extrahieren und öffnen Sie Model.bim mit Dekomprimierungstools

3. **Mit DAX Studio** (für Measure-Analyse):
   - Laden Sie DAX Studio herunter
   - Starten Sie DAX Studio über den Tab "Externe Tools" in Power BI Desktop, um sich direkt mit dem Modell zu verbinden.
